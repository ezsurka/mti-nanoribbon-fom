...
def optimize_parameters(a, a_z, W, T, mu, m_z, params, sym, h_mode, p_mode):
...
def get_S_imp(seed, a, syst, correlation_length):
    ...
    def S_imp(site, Smag_imp):
        ind_x, ind_y = site.tag - np.amin(syst_index, 0)
        return Smag_imp * disorder_box[ind_x, ind_y]
...
def get_bands_topological(a, a_z, T, W, delta, flux, m_z, mu, m0, m1, D, h_mode, get_delta=False, p_mode=0):
    ...
    data_bands = calculate_lead_spectrum(a, a_z, T, W, k, 0, flux, m_z, mu, u_B, u_T, m0, m1, D, 0, 120, 0, h_mode)

def get_fig_of_merit(a, a_z, T, W, delta, m_z, mu, h_mode, u_B=0, u_T=0, get_delta=False, p_mode=0, k_points=1000, num_bands=50, sym=False, **kwargs):
    ...
    a, a_z, W, T, mu, m_z, params, h_mode, p_mode = optimize_parameters(a=a, a_z=a_z, W=W, T=T, mu=mu, m_z=m_z, params=params, sym=sym, h_mode=h_mode, p_mode=p_mode)
    ...
    params['delta'] = delta
    params['S_imp'] = 0
    if   p_mode == 15 and u_B != 0 and u_T != 0:
        mu = mu - 0.1 - max(0,(u_B/2 + u_T/2 - .1)*(70-T)/50)
    elif p_mode == 1  and u_B != 0 and u_T != 0:
        mu = mu - (u_B/2 + u_T/2 - .05)
    if h_mode == 0 or h_mode == 3:
        mu = mu + m_z/2
    ...
    for k in ks:
        data = calculate_lead_spectrum(
            a=a, a_z=a_z, T=T, W=W, k=k, delta=delta, flux=flux, m_z=m_z, mu=mu, u_B=u_B, u_T=u_T,
...
#    return ks, Es
    ...
    if h_mode != 1 and u_B == 0 and u_T == 0 :
...
def finite_spectrum(
        ...
        correlation_length, n_av, k, sum=False, **kwargs):
    params = default_params.copy()
...
def finite_spectrum_deltamu(
    ...
    for i in range(n_av):
        params['mu_ti'] = mu+np.random.default_rng(i).uniform(-deltamu,deltamu,None)
        if Smag_imp == 0:
            params["S_imp"] = lambda site, Smag_imp: 0
        else:
            params["S_imp"] = get_S_imp(0, a, fsyst, correlation_length)
        solver_start = time.perf_counter()
        energies, wfs = spectrum(
            syst=fsyst, params=params, k=k, solver='mumps'
        )
